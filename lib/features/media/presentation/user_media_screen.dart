    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [Tab(text: 'Фото'), Tab(text: 'Видео'), Tab(text: 'Файлы'), Tab(text: 'Голосовые')],
        ),
      ),
      body: TabBarView(
        controller: _tabController,
...
class _MediaList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (docs.isEmpty) return const Center(child: Text('Нет элементов'));
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return FutureBuilder<File?>(
              future: cache.getOrConvert(messageId, data),
  ...
  void _showFullScreen(BuildContext context, List<File> files, int index) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => FullScreenMediaViewer(mediaFiles: files, initialIndex: index)));
  }
}